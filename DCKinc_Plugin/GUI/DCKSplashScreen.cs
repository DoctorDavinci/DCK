...
namespace DCKinc
{
    ...
    class DCKSplashScreen : MonoBehaviour
    {
        private static float MAX_TIP_TIME = 4; // seconds

        private static readonly string[] DCK_TIP1 =
        {
            "Configuring DCK Shields ..."
        };

        /// <summary>
        /// Snark's sneaky little way of thanking various users of this mod for helpful contributions.
        /// </summary>
        private static readonly string[] DCK_TIP2 =
        {
            "Thanx to SpannerMonkey(smce) & TheDog"      // Thanx for everything      
        };
        ...
        private static void InsertTips(LoadingScreen.LoadingScreenState state)
        {
            ...
            tipsList.AddRange(DCK_TIP1);
            int numThanks = 1 + (int)Mathf.Sqrt(DCK_TIP2.Length);
            System.Random random = new System.Random(System.DateTime.UtcNow.Second);
            for (int i = 0; i < numThanks; ++i)
            {
                tipsList.Add(string.Format(DCK_TIP2[random.Next(DCK_TIP2.Length)]));
            }
